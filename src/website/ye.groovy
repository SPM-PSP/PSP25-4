...
<body>
        ...
        <div class="left-panel">
            <div class="model-selector">
                ...
                <select id="modelSelect">
                    ...
                    <option value="falcon-7b/falcon-7b-instruct">Falcon-7b/Falcon-7B-Instruct</option>
                    <option value="gpt-neo-2.7b">GPT-Neo-2.7B</option>
                    <option value="davinci-002" selected>Davinci-002</option>
    ...
    <script>
        ...
        async function readPdfFile(file) {
            alert('PDF解析需要额外库支持，当前仅演示文本文件上传');
            return 'PDF内容解析需要集成PDF.js等库';
        }

        // 读取Word文件 (简单实现，实际应用中可能需要docx.js等库)
        async function readWordFile(file) {
            alert('Word文档解析需要额外库支持，当前仅演示文本文件上传');
            return 'Word内容解析需要集成docx.js等库';
        }
        ...
        async function analyzeText(text, model) {
            try {
                const detectorMap = {
                    //'falcon-7b': 'fast-detect(falcon-7b)',
                    //'falcon-7b-instruct': 'fast-detect(falcon-7b-instruct)',
